...
- [ ] P2-TXN-002 会话级事务上下文（状态机：idle/active/committing/aborted）
- [ ] P2-TXN-003 事务内写集暂存（insert/update/delete）
...
- [ ] P2-LOG-001 事务日志结构定义（txnId, writeSet, pre/post image, checksum）
- [ ] P2-LOG-002 写前日志（WAL）落盘/持久化策略与恢复入口
- [ ] P2-LOG-003 提交批处理器：将事务变更聚合成一次链上写入
- [ ] P2-LOG-004 失败补偿：提交中断可重放/可回滚/幂等重试
...
- [ ] P2-ISO-005 不可重复读行为与文档（RC 允许）
- [ ] P2-ISO-006 并发写冲突检测与标准化错误码

## P2-D. 持久性与一致性（Walrus 版本化特性）

- [ ] P2-DUR-001 提交生成新版本存储对象（immutable object）
...
- [ ] P2-FK-005 ON DELETE RESTRICT/NO ACTION 实现
- [ ] P2-FK-006 ON UPDATE CASCADE/RESTRICT 策略实现
- [ ] P2-FK-007 FK 环与级联深度保护（防止无限级联）

## P2-F. 执行器与存储联动

- [ ] P2-EXE-001 执行计划接入事务上下文（读已提交 + 本事务写）
- [ ] P2-EXE-002 索引/约束检查在事务提交点二次验证
- [ ] P2-EXE-003 批量提交与回放路径统一（线上/恢复同逻辑）
- [ ] P2-EXE-004 事务统计与可观测（txn latency, lock wait, abort ratio）

## P2-G. 测试体系（事务与一致性专项）

- [ ] P2-TEST-001 单测：事务状态机、WAL 编解码、冲突检测
- [ ] P2-TEST-002 并发集成：多会话 RC 语义（脏读/写冲突/回滚可见性）
...
- [ ] P2-BENCH-001 TPC-C 类最小工作负载实现（仓库本地可重复运行）
