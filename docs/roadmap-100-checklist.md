...
- [ ] P4-CTE-001 `WITH` 基础能力（多 CTE、作用域、引用顺序）
- [ ] P4-CTE-002 CTE 与主查询/子查询/视图展开的绑定一致性
- [ ] P4-RCTE-001 `WITH RECURSIVE` 语法与执行骨架
- [ ] P4-RCTE-002 递归锚点/递归步语义校验（列数、类型对齐）
- [ ] P4-RCTE-003 递归终止条件与最大深度保护（防死循环）
- [ ] P4-RCTE-004 层次化查询样例（树/路径）与结果稳定性
- [ ] P4-RCTE-005 递归执行可观测性（迭代次数、行数、耗时）
...
- [ ] P4-SEC-001 `GRANT` / `REVOKE` 语法与权限模型（表/视图/模式最小集）
- [ ] P4-SEC-002 用户与角色基础管理（若 SDK 面向多用户）
- [ ] P4-SEC-003 权限校验链路接入（解析->计划->执行）
- [ ] P4-SEC-004 权限元数据可查询与审计日志基础
- [ ] P4-SEC-005 权限回归矩阵（越权/误拒绝/继承冲突）

## P4-F. 测试体系（SQL-92 核心一致性专项）

- [ ] P4-TEST-001 单测：窗口函数语义矩阵（排序/并列/NULL/边界）
- [ ] P4-TEST-002 单测：`FILTER` + `CASE` 聚合语义矩阵
- [ ] P4-TEST-003 集成：CTE（多层引用、作用域、重名冲突）
- [ ] P4-TEST-004 集成：递归 CTE（层次查询、终止条件、深度上限）
- [ ] P4-TEST-005 集成：游标生命周期与逐行处理
- [ ] P4-TEST-006 集成：PREPARE/EXECUTE 动态 SQL 参数绑定与错误码
- [ ] P4-TEST-007 集成：information_schema 元数据一致性回归
- [ ] P4-TEST-008 回归：新能力与既有 P1/P2/P3 语义兼容
- [ ] P4-TEST-009 标准套件：SQL-92 核心一致性测试接入与报告归档
