...
| Area | Status | Notes |
|---|---|---|
| SELECT/WHERE/ORDER/LIMIT/OFFSET | âœ?| Baseline stable + 3VL filter semantics + expression first-cut (`+ - * / %`, CASE, COALESCE, NULLIF, CAST) + dialect gating for TOP/FETCH/LIMIT clause-shape by profile |
| Aggregates + GROUP BY/HAVING | âœ?| COUNT/SUM/AVG/MIN/MAX |
| JOIN | ðŸŸ¡ | INNER/LEFT/RIGHT (first-cut) |
| Subquery | ðŸŸ¡ | IN/EXISTS/scalar/ANY/ALL first-cut + correlated WHERE refs via `outer.<col>` + FROM subquery first-cut; scalar MIN-subquery NULL semantics aligned for current matrix |
| UNION | ðŸŸ¡ | UNION / UNION ALL first-cut |
| Window | ðŸŸ¡ | ROW_NUMBER first-cut |
| NULL/LIKE | âœ?| 3VL, IS NULL/IS NOT NULL, LIKE/NOT LIKE, LIKE ESCAPE + postgres `ILIKE` dialect-gated |
| Distinctness predicates | âœ?| IS DISTINCT FROM / IS NOT DISTINCT FROM |
| Boolean truth predicates | âœ?| IS [NOT] TRUE/FALSE/UNKNOWN |
| Transaction semantics | â?| Not implemented |
| Full SQL parser/AST | â?| Lightweight parser, not SQL-complete |
| Cost-based optimizer | â?| Not implemented |
...
## Phase A-87 snapshot (2026-03-14)

- Added regression lock for thin-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2009.\u2009amount = ..., u\u2009.\u2009tier = ...`
  - `DELETE o,\u2009u FROM ...`
- Confirms deterministic behavior boundary:
  - thin-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - thin-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
- Included phaseA87 in grouped semantic runner.

## Phase A-88 snapshot (2026-03-14)

- Added regression lock for narrow no-break space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u202F.\u202Famount = ..., u\u202F.\u202Ftier = ...`
  - `DELETE o,\u202Fu FROM ...`
- Confirms deterministic behavior boundary:
  - narrow no-break space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - narrow no-break space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
- Included phaseA88 in grouped semantic runner.

## Phase A-89 snapshot (2026-03-14)

- Added regression lock for hair-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u200A.\u200Aamount = ..., u\u200A.\u200Atier = ...`
  - `DELETE o,\u200Au FROM ...`
- Confirms deterministic behavior boundary:
  - hair-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - hair-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
...
## Phase A-93 snapshot (2026-03-14)

- Added regression lock for punctuation-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2008.\u2008amount = ..., u\u2008.\u2008tier = ...`
  - `DELETE o,\u2008u FROM ...`
- Confirms deterministic behavior boundary:
  - punctuation-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - punctuation-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
- Included phaseA93 in grouped semantic runner.

## Phase A-94 snapshot (2026-03-14)

- Added regression lock for six-per-em-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2006.\u2006amount = ..., u\u2006.\u2006tier = ...`
  - `DELETE o,\u2006u FROM ...`
- Confirms deterministic behavior boundary:
  - six-per-em-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - six-per-em-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
- Included phaseA94 in grouped semantic runner.

## Phase A-95 snapshot (2026-03-14)

- Added regression lock for three-per-em-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2004.\u2004amount = ..., u\u2004.\u2004tier = ...`
  - `DELETE o,\u2004u FROM ...`
- Confirms deterministic behavior boundary:
  - three-per-em-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - three-per-em-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
...
## Phase A-97 snapshot (2026-03-14)

- Added regression lock for em-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2003.\u2003amount = ..., u\u2003.\u2003tier = ...`
  - `DELETE o,\u2003u FROM ...`
- Confirms deterministic behavior boundary:
  - em-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - em-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
- Included phaseA97 in grouped semantic runner.

## Phase A-98 snapshot (2026-03-14)

- Added regression lock for en-space target-token shapes in join-aware DML boundary:
  - `UPDATE ... SET o\u2002.\u2002amount = ..., u\u2002.\u2002tier = ...`
  - `DELETE o,\u2002u FROM ...`
- Confirms deterministic behavior boundary:
  - en-space target-token SET currently rejects with `ERR_UNSUPPORTED_UPDATE`
  - en-space multi-target DELETE is rejected with `ERR_UNSUPPORTED_DELETE`
  - both paths keep constraint-cost counters unchanged.
...
## Phase A-100 snapshot (2026-03-14)

- Added regression lock for em-quad-space target-token shapes in join-aware DML boundary:
  - UPDATE ... SET o\u2001.\u2001amount = ..., u\u2001.\u2001tier = ...
  - DELETE o,\u2001u FROM ...
- Confirms deterministic behavior boundary:
  - em-quad-space target-token SET currently rejects with ERR_UNSUPPORTED_UPDATE
  - em-quad-space multi-target DELETE is rejected with ERR_UNSUPPORTED_DELETE
  - both paths keep constraint-cost counters unchanged.
...
## Phase A-103 snapshot (2026-03-15)

- Added regression lock for zero-width-space target-token shapes in join-aware DML boundary:
  - UPDATE ... SET o\u200B.\u200Bamount = ..., u\u200B.\u200Btier = ...
  - DELETE o,\u200Bu FROM ...
- Confirms deterministic behavior boundary:
  - zero-width-space target-token SET currently rejects with ERR_UNSUPPORTED_UPDATE
  - zero-width-space multi-target DELETE is rejected with ERR_UNSUPPORTED_DELETE
  - both paths keep constraint-cost counters unchanged.
...
## Phase A-105 snapshot (2026-03-15)

- Added regression lock for zero-width-joiner target-token shapes in join-aware DML boundary:
  - UPDATE ... SET o\u200D.\u200Damount = ..., u\u200D.\u200Dtier = ...
  - DELETE o,\u200Du FROM ...
- Confirms deterministic behavior boundary:
  - zero-width-joiner target-token SET currently rejects with ERR_UNSUPPORTED_UPDATE
  - zero-width-joiner multi-target DELETE is rejected with ERR_UNSUPPORTED_DELETE
  - both paths keep constraint-cost counters unchanged.
- Included phaseA105 in grouped semantic runner.

## Phase A-106 snapshot (2026-03-15)

- Added regression lock for word-joiner target-token shapes in join-aware DML boundary:
  - UPDATE ... SET o\u2060.\u2060amount = ..., u\u2060.\u2060tier = ...
  - DELETE o,\u2060u FROM ...
- Confirms deterministic behavior boundary:
  - word-joiner target-token SET currently rejects with ERR_UNSUPPORTED_UPDATE
  - word-joiner multi-target DELETE is rejected with ERR_UNSUPPORTED_DELETE
  - both paths keep constraint-cost counters unchanged.
