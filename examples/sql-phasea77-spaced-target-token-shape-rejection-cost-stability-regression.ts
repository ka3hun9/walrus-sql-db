...
async function main() {
  ...
  // spaced tokenized target fields in join-aware UPDATE currently reject deterministically
  await expectErr(
  ...
  // spaced target list in multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea77-spaced-target-token-shape-rejection-cost-stability-regression ok");
}
