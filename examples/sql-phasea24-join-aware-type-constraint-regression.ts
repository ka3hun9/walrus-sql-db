...
async function main() {
  ...
  // type constraints still enforced on join-aware UPDATE writes
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET u.tier = 'oops' WHERE o.amount = 200"),
    "ERR_TYPE_CONSTRAINT",
  );

  // valid typed write should still pass
  await db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET u.tier = 9 WHERE o.amount = 200");
  ...
  console.log("sql-phasea24-join-aware-type-constraint-regression ok");
}
