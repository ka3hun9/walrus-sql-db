...
async function main() {
  ...
  await expectErr(
    () => db.execute("UPDATE users u LEFT OUTER JOIN orders o ON u.id = o.user_id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u LEFT OUTER JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea43-join-aware-left-outer-join-cost-stability-regression ok");
}
