...
async function main() {
  ...
  assert.ok(usersCost.conflictChecks > 0, `expected users.conflictChecks > 0, got ${usersCost.conflictChecks}`);
  assert.equal(usersCost.rebuildOps, 0, `expected users.rebuildOps=0, got ${usersCost.rebuildOps}`);

  // right table must stay untouched even on left-table conflict path
  ...
  console.log("sql-phasea37-join-aware-conflict-cost-isolation-regression ok");
}
