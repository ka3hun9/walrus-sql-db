...
async function main() {
  ...
  // double-quoted targets in join-aware UPDATE currently reject deterministically as unsupported shape
  await expectErr(
  ...
  // double-quoted multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea74-doublequote-target-shape-rejection-cost-stability-regression ok");
}
