...
async function main() {
  ...
  await expectErr(
    () => db.execute("DELETE FROM users USING users JOIN orders ON users.id = orders.user_id WHERE orders.amount = 100"),
  ...
  console.log("sql-phasea46-unsupported-alt-dml-shape-cost-stability-regression ok");
}
