...
async function main() {
  ...
  // table-name right target with aliased right table should reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET orders.amount = 999, u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
    "SET target must reference left table/alias: orders.amount",
  );

  // mixed table-name + alias multi-target delete should reject deterministically
  await expectErr(
    () => db.execute("DELETE orders, u FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea69-right-table-name-with-right-alias-rejection-cost-stability-regression ok");
}
