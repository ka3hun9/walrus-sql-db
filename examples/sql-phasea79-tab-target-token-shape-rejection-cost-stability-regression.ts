...
async function main() {
  ...
  // tab-broken target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // tab-broken target list in multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea79-tab-target-token-shape-rejection-cost-stability-regression ok");
}
