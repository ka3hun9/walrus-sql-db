...
async function main() {
  ...
  // narrow no-break space target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // narrow no-break space multi-target DELETE list should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea88-nnbsp-target-token-shape-rejection-cost-stability-regression ok");
}
