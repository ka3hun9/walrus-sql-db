...
async function main() {
  ...
  // six-per-em-space target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // six-per-em-space multi-target DELETE list should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea94-sixperemspace-target-token-shape-rejection-cost-stability-regression ok");
}
