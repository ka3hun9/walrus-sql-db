...
async function main() {
  ...
  // vertical-tab-broken target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o\v.\vamount = 999, u\v.\vtier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // vertical-tab-broken target list in multi-target DELETE should reject deterministically
  await expectErr(
    () => db.execute("DELETE o,\v u FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea83-vtab-target-token-shape-rejection-cost-stability-regression ok");
}
