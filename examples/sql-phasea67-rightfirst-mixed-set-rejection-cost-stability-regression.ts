...
async function main() {
  ...
  // right-target first in mixed SET should still deterministically reject
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o.amount = 999, u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
    "SET target must reference left table/alias: o.amount",
  );

  // multi-target delete with reversed target order should reject deterministically
  await expectErr(
    () => db.execute("DELETE o, u FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea67-rightfirst-mixed-set-rejection-cost-stability-regression ok");
}
