...
async function main() {
  ...
  // left table-name + right alias mixed SET currently fails on deterministic type-constraint path
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET users.tier = 9, o.amount = 999 WHERE o.amount = 100"),
    "ERR_TYPE_CONSTRAINT",
  );
  ...
  console.log("sql-phasea72-left-tablename-plus-right-alias-mix-boundary-cost-stability-regression ok");
}
