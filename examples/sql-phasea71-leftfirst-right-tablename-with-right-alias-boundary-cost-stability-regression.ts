...
async function main() {
  ...
  // left-first then right table-name target (while right side is aliased) currently fails on deterministic type-constraint path
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET u.tier = 9, orders.amount = 999 WHERE o.amount = 100"),
    "ERR_TYPE_CONSTRAINT",
  );

  // mixed left table-name + right alias multi-target delete should reject deterministically
  await expectErr(
    () => db.execute("DELETE users, o FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea71-leftfirst-right-tablename-with-right-alias-boundary-cost-stability-regression ok");
}
