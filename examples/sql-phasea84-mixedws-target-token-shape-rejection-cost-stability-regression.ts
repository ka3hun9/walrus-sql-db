...
async function main() {
  ...
  // mixed tab+newline around dot target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o \t.\n amount = 999, u \t.\n tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // mixed tab+newline in multi-target DELETE list should reject deterministically
  await expectErr(
    () => db.execute("DELETE o,\t\n u FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea84-mixedws-target-token-shape-rejection-cost-stability-regression ok");
}
