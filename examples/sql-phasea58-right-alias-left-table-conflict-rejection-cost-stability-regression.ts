...
async function main() {
  ...
  // right alias conflicts with left table name => should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders users ON u.id = users.user_id SET u.tier = 9 WHERE users.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders users ON u.id = users.user_id WHERE users.amount = 100"),
  ...
  console.log("sql-phasea58-right-alias-left-table-conflict-rejection-cost-stability-regression ok");
}
