...
async function main() {
  ...
  // mixed table-name targets (no aliases) currently fail on deterministic type-constraint path
  await expectErr(
    () => db.execute("UPDATE users JOIN orders ON users.id = orders.user_id SET users.tier = 9, orders.amount = 999 WHERE orders.amount = 100"),
    "ERR_TYPE_CONSTRAINT",
  );

  // multi-target table-name DELETE should be rejected deterministically
  await expectErr(
    () => db.execute("DELETE users, orders FROM users JOIN orders ON users.id = orders.user_id WHERE orders.amount = 100"),
  ...
  console.log("sql-phasea66-mixed-table-target-noalias-boundary-cost-stability-regression ok");
}
