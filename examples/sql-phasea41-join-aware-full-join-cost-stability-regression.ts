...
async function main() {
  ...
  await expectErr(
    () => db.execute("UPDATE users u FULL JOIN orders o ON u.id = o.user_id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u FULL JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea41-join-aware-full-join-cost-stability-regression ok");
}
