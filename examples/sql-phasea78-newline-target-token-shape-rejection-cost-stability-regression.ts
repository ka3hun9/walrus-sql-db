...
async function main() {
  ...
  // newline-broken target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o\n.\namount = 999, u\n.\ntier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // newline-broken target list in multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea78-newline-target-token-shape-rejection-cost-stability-regression ok");
}
