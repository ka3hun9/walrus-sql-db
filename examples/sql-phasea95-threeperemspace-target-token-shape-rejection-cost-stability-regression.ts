...
async function main() {
  ...
  // three-per-em-space target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // three-per-em-space multi-target DELETE list should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea95-threeperemspace-target-token-shape-rejection-cost-stability-regression ok");
}
