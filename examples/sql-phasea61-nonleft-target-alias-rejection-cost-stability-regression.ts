...
async function main() {
  ...
  // non-left target alias (`o`) should be rejected deterministically in join-aware DML
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o.amount = 999 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE o FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea61-nonleft-target-alias-rejection-cost-stability-regression ok");
}
