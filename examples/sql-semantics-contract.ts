...
const inferencer: TypeInferencer = {
  inferExprType(expr: ExprAst) {
    if (expr.kind === "literal" && typeof expr.value === "number") return "INT";
    if (expr.kind === "literal" && typeof expr.value === "string") return "TEXT";
    return "UNKNOWN";
  },
};
...
const truth: TruthEvaluator = {
  evalPredicate(expr: ExprAst) {
    if (expr.kind === "literal" && expr.value === true) return "TRUE";
    if (expr.kind === "literal" && expr.value === false) return "FALSE";
    return "UNKNOWN";
  },
};
...
const t1 = contracts.typeInferencer.inferExprType({ kind: "literal", value: 42 }, ctx);
assert.equal(t1, "INT");

const tv = contracts.truthEvaluator.evalPredicate({ kind: "literal", value: null }, {});
