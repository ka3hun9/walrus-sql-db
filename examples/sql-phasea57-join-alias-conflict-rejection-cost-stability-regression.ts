...
async function main() {
  ...
  // conflicting join aliases should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders u ON u.id = u.user_id SET u.tier = 9 WHERE u.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders u ON u.id = u.user_id WHERE u.amount = 100"),
  ...
  console.log("sql-phasea57-join-alias-conflict-rejection-cost-stability-regression ok");
}
