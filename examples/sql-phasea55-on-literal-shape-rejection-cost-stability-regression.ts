...
async function main() {
  ...
  // ON field-to-literal shape is intentionally unsupported in join-aware DML
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = 1 SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders o ON u.id = 1 WHERE o.amount = 100"),
  ...
  console.log("sql-phasea55-on-literal-shape-rejection-cost-stability-regression ok");
}
