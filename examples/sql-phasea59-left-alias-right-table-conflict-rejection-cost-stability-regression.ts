...
async function main() {
  ...
  // left alias conflicts with right table name => should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users orders JOIN orders o ON orders.id = o.user_id SET orders.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE orders FROM users orders JOIN orders o ON orders.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea59-left-alias-right-table-conflict-rejection-cost-stability-regression ok");
}
