...
async function main() {
  ...
  // mixed table-name targets in UPDATE currently fail on deterministic type-constraint path
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET users.tier = 9, orders.amount = 999 WHERE o.amount = 100"),
    "ERR_TYPE_CONSTRAINT",
  );

  // mixed alias/table multi-target DELETE should be rejected deterministically
  await expectErr(
  ...
  console.log("sql-phasea65-mixed-table-target-boundary-cost-stability-regression ok");
}
