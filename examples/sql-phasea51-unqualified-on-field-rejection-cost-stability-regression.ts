...
async function main() {
  ...
  // unqualified ON fields should stay on supported hot path when unambiguous
  await db.execute("UPDATE users u JOIN orders o ON id = user_id SET u.tier = 9 WHERE o.amount = 100");
  await db.execute("DELETE u FROM users u JOIN orders o ON id = user_id WHERE o.amount = 100");
  ...
  console.log("sql-phasea51-unqualified-on-field-rejection-cost-stability-regression ok");
}
