...
async function main() {
  ...
  // non-left target table name (`orders`) should be rejected deterministically in join-aware DML
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET orders.amount = 999 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE orders FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea63-nonleft-target-table-rejection-cost-stability-regression ok");
}
