...
async function main() {
  ...
  // cross-side ON prefixes should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON o.user_id = o.id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders o ON u.id = u.id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea54-on-prefix-cross-side-rejection-cost-stability-regression ok");
}
