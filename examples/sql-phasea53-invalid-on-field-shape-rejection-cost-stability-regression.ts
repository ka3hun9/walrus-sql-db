...
async function main() {
  ...
  // invalid ON field shape (3-part identifier) should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id.extra = o.user_id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders o ON u.id = o.user_id.extra WHERE o.amount = 100"),
  ...
  console.log("sql-phasea53-invalid-on-field-shape-rejection-cost-stability-regression ok");
}
