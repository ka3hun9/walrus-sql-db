...
async function main() {
  ...
  // ON expression (non field=field) shape should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON ABS(u.id) = o.user_id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders o ON u.id = ABS(o.user_id) WHERE o.amount = 100"),
  ...
  console.log("sql-phasea56-on-expression-shape-rejection-cost-stability-regression ok");
}
