...
async function main() {
  ...
  await db.execute("INSERT INTO orders (id, user_id, amount) VALUES (1, 1, 100)");

  db.resetConstraintIndexCost("users");
  db.resetConstraintIndexCost("orders");

  // same-name unqualified ON fields currently route to supported hot path
  await db.execute("UPDATE users u JOIN orders o ON id = id SET u.tier = 9 WHERE o.amount = 100");
  await db.execute("DELETE u FROM users u JOIN orders o ON id = id WHERE o.amount = 100");
  ...
  console.log("sql-phasea52-unqualified-same-name-on-hot-path-cost-regression ok");
}
