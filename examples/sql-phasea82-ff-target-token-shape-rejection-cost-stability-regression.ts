...
async function main() {
  ...
  // form-feed-broken target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o\f.\famount = 999, u\f.\ftier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // form-feed-broken target list in multi-target DELETE should reject deterministically
  await expectErr(
    () => db.execute("DELETE o,\f u FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea82-ff-target-token-shape-rejection-cost-stability-regression ok");
}
