...
async function main() {
  ...
  // reversed ON-side ordering is intentionally unsupported at this stage
  await expectErr(
    () => db.execute("UPDATE users u INNER JOIN orders o ON o.user_id = u.id SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u INNER JOIN orders o ON o.user_id = u.id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea50-inner-join-on-side-symmetry-cost-regression ok");
}
