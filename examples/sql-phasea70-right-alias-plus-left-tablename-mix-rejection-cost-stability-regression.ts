...
async function main() {
  ...
  // right alias target + left table-name target mix should reject on first right-side target deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o.amount = 999, users.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
    "SET target must reference left table/alias: o.amount",
  );

  // mixed alias+table-name multi-target delete should reject deterministically
  await expectErr(
    () => db.execute("DELETE o, users FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea70-right-alias-plus-left-tablename-mix-rejection-cost-stability-regression ok");
}
