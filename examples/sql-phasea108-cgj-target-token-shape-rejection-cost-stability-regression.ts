...
async function main() {
  ...
  // combining grapheme joiner target tokens normalize and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o\u034F.\u034Famount = 999, u\u034F.\u034Ftier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // combining grapheme joiner multi-target DELETE list should reject deterministically
  await expectErr(
    () => db.execute("DELETE o,\u034Fu FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea108-cgj-target-token-shape-rejection-cost-stability-regression ok");
}
