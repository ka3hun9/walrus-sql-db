...
async function main() {
  ...
  await expectErr(
    () => db.execute("UPDATE users u NATURAL JOIN orders o SET u.tier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE u FROM users u NATURAL JOIN orders o WHERE o.amount = 100"),
  ...
  console.log("sql-phasea49-natural-join-cost-stability-regression ok");
}
