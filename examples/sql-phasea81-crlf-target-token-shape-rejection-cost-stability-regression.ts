...
async function main() {
  ...
  // CRLF-broken target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET o\r\n.\r\namount = 999, u\r\n.\r\ntier = 9 WHERE o.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // CRLF-broken target list in multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea81-crlf-target-token-shape-rejection-cost-stability-regression ok");
}
