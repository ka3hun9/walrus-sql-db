...
async function main() {
  ...
  // non-breaking-space target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // non-breaking-space multi-target DELETE list should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea86-nbsp-target-token-shape-rejection-cost-stability-regression ok");
}
