...
async function main() {
  ...
  // right-side multi-match for same left-row target
  ...
  assert.equal(usersCost.conflictChecks, 0, `expected users.conflictChecks=0 on delete path, got ${usersCost.conflictChecks}`);
  assert.equal(usersCost.rebuildOps, 0, `expected users.rebuildOps=0 on delete path, got ${usersCost.rebuildOps}`);

  // right table must stay untouched on left-target delete path
  ...
  console.log("sql-phasea38-join-aware-delete-cost-isolation-regression ok");
}
