...
async function main() {
  ...
  // conflict path should fail deterministically and keep state stable
  await expectErr(
  ...
  assert.equal(cost.deleteOps, 0, `expected deleteOps=0 on update-conflict path, got ${cost.deleteOps}`);
  assert.equal(cost.rebuildOps, 0, `expected rebuildOps=0 on update-conflict path, got ${cost.rebuildOps}`);

  console.log("sql-phasea35-join-aware-conflict-state-stability-regression ok");
}
