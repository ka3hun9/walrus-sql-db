...
function expectErr(fn: () => unknown | Promise<unknown>, code: string, contains?: string) {
...
async function main() {
  ...
  // right-target first in mixed SET (table names, no aliases) should reject deterministically
  await expectErr(
    () => db.execute("UPDATE users JOIN orders ON users.id = orders.user_id SET orders.amount = 999, users.tier = 9 WHERE orders.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
    "SET target must reference left table/alias: orders.amount",
  );

  // multi-target delete with reversed table-name order should reject deterministically
  await expectErr(
    () => db.execute("DELETE orders, users FROM users JOIN orders ON users.id = orders.user_id WHERE orders.amount = 100"),
  ...
  console.log("sql-phasea68-rightfirst-mixed-table-set-rejection-cost-stability-regression ok");
}
