...
async function main() {
  ...
  // mixed-target SET currently deterministically fails with type-constraint parsing path
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.id = o.user_id SET u.tier = 9, o.amount = 999 WHERE o.amount = 100"),
    "ERR_TYPE_CONSTRAINT",
  );

  // multi-target DELETE must be rejected deterministically
  await expectErr(
    () => db.execute("DELETE u, o FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount = 100"),
  ...
  console.log("sql-phasea64-mixed-target-and-multidelete-rejection-cost-stability-regression ok");
}
