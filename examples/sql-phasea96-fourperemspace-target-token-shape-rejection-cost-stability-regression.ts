...
async function main() {
  ...
  // four-per-em-space target tokens normalize to spaced token shape and reject deterministically
  await expectErr(
  ...
  // four-per-em-space multi-target DELETE list should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea96-fourperemspace-target-token-shape-rejection-cost-stability-regression ok");
}
