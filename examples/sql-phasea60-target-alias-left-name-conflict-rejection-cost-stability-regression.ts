...
async function main() {
  ...
  // left table name used as right alias plus left alias used in target should be rejected deterministically
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders users ON u.id = users.user_id SET users.tier = 9 WHERE users.amount = 100"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  await expectErr(
    () => db.execute("DELETE users FROM users u JOIN orders users ON u.id = users.user_id WHERE users.amount = 100"),
  ...
  console.log("sql-phasea60-target-alias-left-name-conflict-rejection-cost-stability-regression ok");
}
