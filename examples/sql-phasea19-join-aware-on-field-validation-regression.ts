...
async function main() {
  ...
  // deterministic ON validation: left ON field must exist on left schema
  await expectErr(
    () => db.execute("UPDATE users u JOIN orders o ON u.nope = o.user_id SET u.tier = 9 WHERE u.id = 2"),
    "ERR_UNSUPPORTED_UPDATE",
  );

  // deterministic ON validation: right ON field must exist on right schema
  await expectErr(
    () => db.execute("DELETE u FROM users u JOIN orders o ON u.id = o.nope WHERE u.id = 2"),
    "ERR_UNSUPPORTED_DELETE",
  );

  // known columns still pass
  ...
  console.log("sql-phasea19-join-aware-on-field-validation-regression ok");
}
