...
async function main() {
  ...
  // single-quoted pseudo-identifiers in join-aware UPDATE currently reject deterministically as unsupported shape
  await expectErr(
  ...
  // single-quoted multi-target DELETE should reject deterministically
  await expectErr(
  ...
  console.log("sql-phasea76-singlequote-target-shape-rejection-cost-stability-regression ok");
}
